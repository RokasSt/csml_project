                  ...
                  for exp in target_file_names.keys():
                      ...
                      if file_name == target_file_names[exp]:
                         ...
                         if exp not in best_runs[sub_f2].keys():
                            ...
                            check_errors[sub_f2][exp] = mean_error
                            
                         else: 
                             
                            if mean_error < check_errors[sub_f2][exp]:
                                
                               best_runs[sub_f2][exp]    = sub_f1
                               
                               check_errors[sub_f2][exp] = mean_error
                                 
print("Runs with lowest reconstruction errors:")
print(best_runs)
...
with open(best_runs_err_path, 'w') as json_file:
     ...
     json.dump(check_errors, json_file)
     
num_algorithms = len(check_errors.keys())
     
dict_of_lists = plot_utils.process_err_dict(means_dict = check_errors,
                                            bar_plots = True)
